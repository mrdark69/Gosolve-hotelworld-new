...
public class Model_PostSeo : BaseModel<Model_PostSeo>
{
    ...
    public Model_PostSeo GetPostSeoByPostID(int PostID)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("SELECT TOP 1 * FROM PostSEO WHERE PostID=@PostID", cn);
...
FacebookImage=@FacebookImage ,TwitterTitle=@TwitterTitle, TwitterDescription=@TwitterDescription,TwitterImages=@TwitterImages,GoogleAnalytic=@GoogleAnalytic  WHERE PostID=@PostID", cn);
            cn.Open();
            cmd.Parameters.Add("@PostID", SqlDbType.Int).Value = seo.PostID;
...
@FacebookDescription,@FacebookImage,@TwitterTitle,@TwitterDescription,@TwitterImages,@GoogleAnalytic) ", cn);

                cmdinsert.Parameters.Add("@PostID", SqlDbType.Int).Value = seo.PostID;
